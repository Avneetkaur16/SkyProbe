...
export const searchFlight = async(req, res) => {
    ...
    try {
        if (category === "business") {
            const flightsArr = await flights.find({ "origin.city": origin, "destination.city": destination, "origin.date": date, "business.seats": { $gt: Number(seats) } });
            res.status(200).json(flightsArr);
        }
        const flightsArr = await flights.find({ "origin.city": origin, "destination.city": destination, "origin.date": date, "economy.seats": { $gt: Number(seats) } });
        res.status(200).json(flightsArr);
