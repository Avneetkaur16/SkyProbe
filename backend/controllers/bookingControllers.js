...
export const newBooking = async(req, res) => {
    ...
    try {
        ...
        if (category === 'Business') {
            await flights.findByIdAndUpdate(flightId, {
                $set: { 'business.seats': Number(f.business.seats) - 1  }
            })

        } else {
            await flights.findByIdAndUpdate(flightId, {
                $set: { 'economy.seats': Number(f.economy.seats) - 1 }
            })
        }
    ...
    try {
        ...
        if (category === "Business") {
            await flights.findByIdAndUpdate(flight._id, {
                $set: { 'business.seats': Number(flight.business.seats) + 1 }
            })
        } else {
            await flights.findByIdAndUpdate(flight._id, {
                $set: { 'economy.seats': Number(flight.business.seats) + 1 }
            })
        }
